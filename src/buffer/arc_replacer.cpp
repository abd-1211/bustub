...
auto ArcReplacer::Evict() -> std::optional<frame_id_t> {
  ...
  if (mru_.size() >= mru_target_size_) {
    // try mru first
    for (auto it = mru_.rbegin(); it != mru_.rend(); ++it) {  // rbegin is reverse iterator starting at last element and
                                                              // rend is reverse iterator staring at first element
      ...
      if (map_it != alive_map_.end() && map_it->second->evictable_) {  // check if evictable
        ...
        mru_.erase(
            std::next(it)
                .base());  // cant use erase directly on reverse iterator so convert it to normal first then erase
    ...
    for (auto it = mfu_.rbegin(); it != mfu_.rend(); ++it) {  // rbegin is reverse iterator starting at last element and
                                                              // rend is reverse iterator staring at first element
      auto map_it = alive_map_.find(*it);
      if (map_it != alive_map_.end() && map_it->second->evictable_) {  // check if evictable
        frame_id_t fid = *it;                                          // derefence the iterator to get fid
  ...
  if (alive_map_.count(frame_id))  // if present in mfu or mru logic
  {
    ...
    if (ghost_map_.count(page_id))  // if present in ghost mfu or mru
    {
      ...
      if (obj->arc_status_ == ArcStatus::MRU_GHOST)  // in mru ghost
      {
        if (mru_ghost_.size() >= mfu_ghost_.size()) {
          mru_target_size_ =
              std::min(++mru_target_size_, replacer_size_);  // target size should not be lower than replacer size
    ...
    } else  // if not present anywhere
    {
      if (mru_.size() + mru_ghost_.size() == replacer_size_)  // mru + mrughost = replacer
      {
        ...
        auto obj = std::make_shared<FrameStatus>(page_id, frame_id, false, ArcStatus::MRU,
                                                 mru_.begin());  // make obj with same struct as the alive map to store
